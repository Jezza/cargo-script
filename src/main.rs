...
#[structopt(name = "cargo", usage = "cargo <SUBCOMMAND>", raw(setting = "AppSettings::SubcommandRequiredElseHelp", bin_name = "\"cargo\""))]
enum SubCommand {
	/// Compiles and runs "Cargoified Rust scripts".
	#[structopt(name = "script", usage = "cargo script [FLAGS OPTIONS] [--] <script> <args>...")]
	Script {
		#[structopt(flatten)]
		args: Args
	},
	/// Manage Cargo Script expression templates.
	#[structopt(name = "templates")]
	Templates {
		#[structopt(subcommand)]
		args: templates::Args
	},
	/// Manage file assocations.
	#[structopt(name = "file-association")]
	#[cfg(windows)]
	FileAssoc {
		#[structopt(subcommand)]
		args: file_assoc::Args
	},
}

#[derive(Debug, StructOpt)]
struct Args {
	/// Script file (with or without extension) to execute.
	...
	/// Execute <script> as a literal expression and display the result.
	#[structopt(long = "expr", short = "e", requires = "script", group = "expr_or_loop", raw(conflicts_with_all = r#"&["loop"]"#))]
	expr: bool,

	/// Execute <script> as a literal closure once for each line from stdin.
	#[structopt(name = "loop", long = "loop", short = "l", requires = "script", group = "expr_or_loop", raw(conflicts_with_all = r#"&["expr"]"#))]
	looping: bool,

	/// Invoke the loop closure with two arguments: line, and line number.
	#[structopt(long = "count", requires = "loop")]
	count: bool,

	/// Specify where to place the generated Cargo package.
	#[structopt(long = "pkg-path", requires = "script", raw(conflicts_with_all = r#"&["clear_cache", "force"]"#))]
	pkg_path: Option<String>,

	/// Generate the Cargo package, but don't compile or run it.
	#[structopt(long = "gen-pkg-only", requires = "script", raw(conflicts_with_all = r#"&["args", "build_only", "debug", "force", "test", "bench"]"#))]
	gen_pkg_only: bool,

	/// Build the script, but don't run it.
	#[structopt(long = "build-only", requires = "script", raw(conflicts_with_all = r#"&["args"]"#))]
	build_only: bool,

	/// Clears out the script cache.
	#[structopt(long = "clear-cache")]
	clear_cache: bool,

	/// Build a debug executable, not an optimised one.
	#[structopt(long = "debug", requires = "script")]
	debug: bool,

	/// Add an additional Cargo dependency.  Each SPEC can be either just the package name (which will assume the latest version) or a full `name=version` spec.
	#[structopt(long = "dep", short = "d", requires = "script")]
	dep: Vec<String>,

	/// Like `dep`, except that it *also* adds a `#[macro_use] extern crate name;` item for expression and loop scripts.  Note that this only works if the name of the dependency and the name of the library it generates are exactly the same.
	#[structopt(long = "dep-extern", short = "D", requires = "expr_or_loop")]
	dep_extern: Vec<String>,

	/// Adds an `#[macro_use] extern crate name;` item for expressions and loop scripts.
	#[structopt(name = "extern", long = "extern", short = "x", requires = "expr_or_loop")]
	extern_: Vec<String>,

	/// Force the script to be rebuilt.
	#[structopt(long = "force", requires = "script")]
	force: bool,

	/// Add a #![feature] declaration to the crate.
	#[structopt(long = "unstable-features", short = "u", requires = "expr_or_loop")]
	unstable_features: Vec<String>,

	/// Override whether or not the shared binary cache will be used for compilation.
	#[structopt(long = "use-shared-binary-cache", requires = "expr_or_loop", raw(possible_values = r#"&["yes", "no"]"#))]
	use_bincache: Option<bool>,

	/// Migrate data from older versions.
	#[structopt(long = "migrate-data", raw(possible_values = r#"&["dry-run", "for-real"]"#))]
	migrate_data: Option<MigrationKind>,

	/// Compile and run tests.
	#[structopt(long = "test", raw(conflicts_with_all = r#"&["bench", "debug", "args", "force"]"#))]
	test: bool, // , conflicts_with = "bench"

	/// Compile and run benchmarks.  Requires a nightly toolchain.
	#[structopt(long = "bench", raw(conflicts_with_all = r#"&["test", "debug", "args", "force"]"#))]
	bench: bool, // , conflicts_with = "test"

	/// Specify a template to use for expression scripts.
	#[structopt(long = "template", short = "t", requires = "expr")]
	template: Option<String>,
}

impl Args {
	pub(crate) fn build_kind(&self) -> BuildKind {
		match (self.test, self.bench) {
...
fn try_main() -> Result<i32> {
	let args: SubCommand = StructOpt::from_args();
	info!("Arguments: {:?}", args);
	...
	let args = match args {
		...
		SubCommand::Templates {
			args
		} => return templates::try_main(args),
		#[cfg(windows)]
		SubCommand::FileAssoc {
			args
		} => return file_assoc::try_main(args),
	};
	...
	*/
	...
	let build_kind = args.build_kind();

	let input = match (args.script, args.expr, args.looping) {
