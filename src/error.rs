...
impl<T: 'static + Error> From<Box<T>> for MainError {
...
pub trait ResultExt {
	...
	fn err_tag<S>(self, msg: S) -> Result<Self::OkType>
...
impl<T> ResultExt for Result<T> {
	...
	fn err_tag<S: Into<Cow<'static, str>>>(self, msg: S) -> Result<Self::OkType> {
		self.map_err(|err| MainError::Tag(err.blame(), msg.into(), Box::new(err)))
	}

	fn shift_blame(self, blame: Blame) -> Self {
		self.map_err(|mut err| {
			err.shift_blame(blame);
			err
		})
	}
}
